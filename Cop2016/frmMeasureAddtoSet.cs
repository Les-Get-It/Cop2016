...
namespace COP2001
{
    public partial class frmMeasureAddToSet : Telerik.WinControls.UI.RadForm
    {

        public frmMeasureAddToSet()
        {
            InitializeComponent();
            ...
            try
            {
                ...
                for (i = 0; i <= lstIndicators.Items.Count - 1; i++)
                {
                    if (lstIndicators.SelectedIndex == i)
                    {
                        ...
                        modGlobal.gv_sql = string.Format("{0} Values ({1}", modGlobal.gv_sql,
                            Support.GetItemData(frmMeasureSetupCopy.cboMeasureSet, frmMeasureSetupCopy.cboMeasureSet.SelectedIndex));
                ...
                frmMeasureSetupCopy.RefreshMeasureSetField();
        ...
        private void frmMeasureAddToSet_Load(object sender, EventArgs e)
        {
            this.CenterToParent();
            try
            {
        ...
        public void RefreshIndicators()
        {
            ...
            try
            {
                ...
                modGlobal.gv_sql = string.Format("{0} where MeasureSetID = {1})", modGlobal.gv_sql,
                    Support.GetItemData(frmMeasureSetupCopy.cboMeasureSet, frmMeasureSetupCopy.cboMeasureSet.SelectedIndex));
